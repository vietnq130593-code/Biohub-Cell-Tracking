...
> **REVIEW #2 (20:50 UTC 23/9): đã verify toàn bộ tham chiếu line-level + thêm patch D bắt buộc** — xem §5.6.
...
| Code phải viết | 6 file theo pattern v13.2: `build-ver14-monolith.py` (**4 delta-patch A-D** — REVIEW #2 thêm D) → `cell-monolith.py` · `make-ver14-ipynb.py` · ktool `--ver 14` · `submit-v14.py` · ledger classification |
...
│   ├─ guard _EXPECTED_NUMERIC 11 hằng (+ E1: 0.4 → 12) · _EXPECTED_TEXT 7 hằng (giữ nguyên v132)
...
|---|---|---|
| 1 | `ver-14/build-ver14-monolith.py` | **4 delta-patch (REVIEW #1 + #2 — xem §5.5/§5.6)**: (A) block `[ver14]` set E1='0.40' chèn sau block [ver132] TRƯỚC guard · (B) thêm `'BIOHUB_DUAL_SEED_EDGE_THRESHOLD': 0.4` vào `_EXPECTED_NUMERIC` · (C) line ~1173 `'0.48'`→`'0.40'` — bắt buộc, nếu không block 1173 (chạy SAU) ghi đè về 0.48 im lặng (bug env-ordering kiểu REVIEW-2 v12) · **(D) line ~5637 `'edge_candidate_threshold': 0.48` → đọc env** (REVIEW #2 — receipt integrity, xem §5.6-1) · py_compile + determinism (build ×2 cùng md5) |
...
**Guard mới — block `[ver14]` + 3 anchor (REVIEW #1 §5.5 + #2 §5.6):**
...
- **(D)** Line ~5637: `'edge_candidate_threshold': 0.48,` → `'edge_candidate_threshold': float(os.environ.get('BIOHUB_DUAL_SEED_EDGE_THRESHOLD', '0.48')),` — receipt `_guard_report['configuration']` phải phản chiếu giá trị chạy thật (anchor duy nhất — đã verify chỉ xuất hiện 1 lần).
...
| CENSUS | forks ∈ [100,200] (kỳ vọng 141-165) · nodes **banked 122,787** ±5% = [116,648, 128,926] · **edges theo audit R5: banked 118,332 ±5% = [112,416, 124,248]** (REVIEW #2 đính chính 112,215→112,416) — E1 tăng edges hợp lệ tới đó; vượt = **FAIL** census HARD (submit tự hủy — R5 là PASS/FAIL, không ack được) | machinery-audit R5 enforcing |
...
## §5.6 REVIEW #2 — receipt integrity + số học R5 (verify code 20:50 UTC 23/9)

Review đã đối chiếu **mọi tham chiếu line-level với code v13.2 thật** (md5 9a0e872a): line 1173 env-set ✅ · read-point monolith 1415 (= predict 818) ✅ · apply 1439 (= predict 842, điều kiện `if secondary_weights_text:` ✅) · predict line 720 `if probs[i, j] > cfg.threshold` ✅ · guard line ~217 với block [ver132] set trước ✅ · leaf-prune 3984 `prob < 0.3` ✅ · R4 [100,200] + R5 ±5% theo ledger ✅ · ack-path b5f131e ✅. **4 đính chính:**

1. **PATCH D bắt buộc (mới)** — `_guard_report` line 5637 chứa `'edge_candidate_threshold': 0.48` **hardcoded literal** (không đọc env). Với A+B+C, runtime chạy 0.40 nhưng receipt in 0.48 → **receipt nói dối** — đúng loại thất bại receipt-integrity từng đầu độc chuỗi chẩn đoán v13.1. Patch D: thay literal bằng `float(os.environ.get('BIOHUB_DUAL_SEED_EDGE_THRESHOLD', '0.48'))` (cùng style các entry lân cận như sef_tta_weight; anchor duy nhất đã verify).
2. **Bằng chứng parser cho tính bắt buộc của C** — `extract_env()` của machinery-audit dùng `ASSIGN_RE` với **last-assignment-wins**. Nếu bỏ C: parser đọc '0.40' (A, sớm) rồi '0.48' (1173, muộn) → effective = '0.48' = banked → **R2 không flag diff nào → audit MÙ với E1 chết im lặng**. C là cơ chế duy nhất giữ cả runtime + audit nhất quán.
3. **Số học R5** — 118,332 × 0.95 = **112,415.4** → integer ≥ 112,416 (draft ghi 112,215 — sai 200 do typo). Nodes: audit đối chiếu **banked 122,787** (không phải census v13.2 122,372). R5 là PASS/**FAIL** (không phải WARN) → vượt band = submit tự hủy, đúng vai trò lưới an toàn E1.
4. **Nit đếm** — `_EXPECTED_NUMERIC` hiện có **11** hằng (draft ghi 12); +E1 = 12 sau patch B.
...
- **Rào cản duy nhất hiện tại: điểm v13.2 còn PENDING (8h40'+, queue Kaggle chậm — kernel COMPLETE, không lỗi). Chờ điểm trước khi push v14** (điều kiện tiên quyết §0).
