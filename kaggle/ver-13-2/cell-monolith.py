...
os.environ['BIOHUB_LOWDET_THRESHOLD'] = '0.0'
os.environ['BIOHUB_LOWDET_DIR'] = '/kaggle/working/lowdet'
os.environ['BIOHUB_READMIT_RADIUS_UM'] = '0.0'
os.environ['BIOHUB_READMIT_MIN_SCORE'] = '0.965'
os.environ['BIOHUB_GAPFILL_MAX_GAP'] = '0'
...
print('[ver12] Phase H portfolio: reparent EP=0.4 | orphan-adopt=1 floor=0.5 | READMIT r=0.0um s>=0.965 | GAPFILL gap<=0 | lowdet>=0.0 | SEF_TTA w=0.75 | DC=0.2 | validator=OFF (ppsweep no-op, save ~90min GPU)' + ' | diverge=-2.0')

# [ver131] Phase J — adjEJ autopsy-driven (22/9: ver-13-1-config.json; base v12.1-knockout).
# Block chạy SAU [ver12] → assignment v131 là CUỐI CÙNG (kỉ luật REVIEW-2;
# builder assert final-env từng key — xem cuối build-ver131-monolith.py).
# SR0: máy division v11 = 0 TP mọi cấp (divJ 0.0000; TP fork-edge 0) → tắt
#   hẳn safe-divisions + reparent: chỉ diệt FP, không mất gì.
# GC3: gap-close 5.0→3.0 — TP-bridge p50 1.68µm vs FP-bridge p50 2.13µm
#   (production đo) → giữ 19/21 TP giết 7/42 FP ≈ +0.0019.
# P1 revert diverge 0.5 + orphan 0 (hành xử v11 0.947) · P2 velocity 0.5
#   GIỮ (0.25 ĐỘC −0.0028 đo được 22/9) · P3 leaf-prune 0.30 (trung tính,
#   hygiene) · P4 KHÔNG divwide (moot với SR0).
os.environ['BIOHUB_SAFE_DIV_DIVERGE_UM'] = '0.5'
...
os.environ['BIOHUB_GAP_CLOSE_UM'] = '3.0'
os.environ['BIOHUB_OUTPUT_SAFE_DIVISIONS'] = '0'
os.environ['BIOHUB_REPARENT_ENABLE'] = '0'
...
# [ver132] Phase J-bis — RESTORE MÁY DIVISION (ERRORS-LEDGER.md L13a/L13c, 23/9).
...
_EXPECTED_NUMERIC = {'BIOHUB_DET_THRESHOLD': 0.965, 'BIOHUB_ILP_APPEARANCE_WEIGHT': 0.0, 'BIOHUB_ILP_DISAPPEARANCE_WEIGHT': 2, 'BIOHUB_GAP_CLOSE_UM': 3.0, 'BIOHUB_OUTPUT_MIN_TRACK_LEN': 6.0, 'BIOHUB_SAFE_DIV_MAX_UM': 9.0, 'BIOHUB_DEEPCENTER_SAFE_DIV_THRESHOLD': 0.2, 'BIOHUB_BIDIRECTIONAL_EDGE_WEIGHT': 0.15, 'BIOHUB_SECONDARY_EDGE_FEATURE_TTA_WEIGHT': 0.75, 'BIOHUB_MOTION_RELINK_VELOCITY_WEIGHT': 0.5, 'BIOHUB_LEAF_PRUNE_MIN_EDGE_PROB': 0.3}
_EXPECTED_TEXT = {'BIOHUB_OUTPUT_SAFE_DIVISIONS': '1', 'BIOHUB_REPARENT_ENABLE': '1', 'BIOHUB_BIDIRECTIONAL_FUSION_MODE': 'harmonic_probability', 'BIOHUB_DUAL_SEED_MIN_CANDIDATE_RETENTION': '0.90', 'BIOHUB_EDGE_FEATURE_TTA': '1', 'BIOHUB_DEEPCENTER_TTA': '1', 'BIOHUB_SECONDARY_EDGE_FEATURE_TTA': '1'}
_drift = {}
...
EXPERIMENT_TAG = 'secondary_deepcenter_tta_0947_reparent_hoct_v132_restore_division_gc3_leaf'
...
# [ver131] Phase J — LEAF-PRUNE (port amanatar v4 nguyên văn; trung tính
# đo được, giữ làm hygiene −75 node rác). 0 = inert. Miễn trừ: con division
# (edge_prob None), node frame cuối, node >1 cạnh vào. Single-pass.
...
def _divnet_rerank_proposals(bundle: dict, dataset, t: int, proposals: list, nodes_by_id: dict,
    ...
    for prop, p_div in zip(proposals, p_divs):
        ...
        out.append((parent_dist + 0.15 * sister_dist - w * float(p_div), source_id, candidate_id, parent_dist, sister_dist, float(p_div), *prop[6:]))
...
# [ver12] ==== NODE-RECALL: READMIT + GAPFILL (port thtennant batch-B — V12-RESEARCH §3.1-3.2) ====
...
# [ver131] ==== P3 LEAF-PRUNE — port nguyên văn amanatar v4 (cell 5 dòng 1365-1421;
# LB 0.948 của họ; đo TRUNG TÍNH trên pipeline mình 22/9 — giữ làm hygiene;
# receipts leaf_prune_nodes/leaf_prune_edges). ====
...
def filter_output_graph(nodes_by_id: dict[int, dict[str, object]], raw_edges: list[dict[str, object]], dataset: str | None = None, deepcenter_bundle: dict[str, object] | None = None, divnet_bundle: dict[str, object] | None = None) -> tuple[dict[int, dict[str, object]], list[dict[str, object]], dict[str, int]]:
    ...
    # [ver131] P3 LEAF-PRUNE — sau short-track, TRƯỚC linefit (đúng vị trí amanatar).
    # Env-gated: LEAF_PRUNE_MIN_EDGE_PROB<=0 → inert.
...
print('Verified progression preserved: 0.933 -> 0.934 -> 0.939 -> 0.941 -> 0.946 -> 0.947 -> ver-8 reparent -> ver-10 hoct-veto mode1 -> ver-11 mutual_nn-off + MIN_PDIV floor -> ver-12 4-truc portfolio (reparent+orphan+readmit+gapfill) -> ver-13.1 adjEJ-autopsy (SR0 — FAIL 0.911, L13) -> ver-13.2 restore-division (L13c: máy division BẬT v11-exact + GC3 + leaf)')